...
arduino-cli compile -b "$FQBN" --build-path "$BUILD" \
  --build-property "compiler.cpp.extra_flags=-DUSER_SETUP_LOADED=1 -include $PWD/past-experiments/firmware/claude_pet/tft_setup.h" \
  past-experiments/firmware/claude_pet

SHA=$(git rev-parse --short HEAD)
git diff --quiet || SHA="$SHA-dirty"
mkdir -p past-experiments/firmware/build-archive
cp "$BUILD/claude_pet.ino.elf" \
   "past-experiments/firmware/build-archive/claude_pet-$SHA-$(date +%Y%m%d-%H%M%S).elf"
echo "archived ELF for $SHA"

launchctl bootout "gui/$(id -u)/com.github.cc-buddy-bridge.daemon" 2>/dev/null || true
sleep 2
arduino-cli upload -p "$PORT" -b "$FQBN" --input-dir "$BUILD" past-experiments/firmware/claude_pet

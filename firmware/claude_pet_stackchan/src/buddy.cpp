...
// ──────────────── shared geometry ────────────────
// Landscape 320x240: pet centred-left so the mini clock (top-left) and the
// transcript HUD (bottom) share the panel. At 2× a 12-char body is 144px
// wide; particles reach ±72px, so x spans 28..172 inside the 200px canvas
// and the strip clear never touches the right-hand side.
const int BUDDY_X_CENTER = 100;
const int BUDDY_CANVAS_W = 200;
...
// 2× on the 320x240 landscape home screen (3× would put the body's last
// row at y=182, under the swipe-card band that starts at 126), 1× in peek.
// Species art is space-padded to a fixed width for alignment at 1×; at >1×
// we trim and re-center per line so the padding doesn't push ink off-screen.
static const uint8_t SCALE_HOME = 2, SCALE_PEEK = 1;
static uint8_t _scale = SCALE_HOME;
...
void buddySetPeek(bool peek) {
  uint8_t s = peek ? SCALE_PEEK : SCALE_HOME;
  if (s == _scale) return;
  _scale = s;
  buddyInvalidate();
}

"""Think out loud: what buddy is told while a learner talks through a math problem.
...
VOICE_RULES = """
[think out loud] The learner asked you to listen while they think out loud about their math problem.
The learner is a child, about 10 years old. From now until listening ends:
...
- Every reply is at most one or two short sentences a 10-year-old can read on your little screen.
...
- Only when they clearly ask to see one step ("show me one step"), delegate it: the step must go through
  the math_lesson tool so the whiteboard shows it. Do not say the step before its result arrives.
...
BACKEND_RULES = """
Think out loud is ON. The learner is a child, about 10, talking through the lesson below.
- Everything the learner says is already appended to the lesson's ideas. Never call math_lesson ideas.
...
- "Is this right?" / "did I make a mistake?" -> math_lesson check, then repeat only its first slip, or
  its praise, in one short sentence.
- "I'm stuck" / "what do I do?" -> math_lesson hint.
- Only an explicit request to see one step -> math_lesson step. Say that step and nothing after it.
- "I'm done", "stop listening" -> math_lesson stop-listening.
...
def listen_refusal(lesson: Optional[Mapping[str, Any]]) -> Optional[str]:
    """Why listening cannot start for this lesson, as a sentence for a child, or None when it can."""
    if not lesson:

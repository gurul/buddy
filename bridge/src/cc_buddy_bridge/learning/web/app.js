...
async function dashboard() {
  ...
  main.innerHTML=`<section class="hero"><div><span class="kicker">a little curiosity goes a long way</span><h1>Big ideas start<br>with little steps.</h1><p>Think out loud, try things, and figure it out together. Your work is saved right here.</p><button class="primary" id="new-main">＋ Start a math lesson</button></div><div class="hero-robot">${ROBOT}</div></section>
...
function renderLesson() {
  ...
  <div class="workspace"><section class="board-panel"><div class="problem-strip"><span class="kicker">${input?'your problem':'let’s try this'}</span><strong>${escapeHTML(s.problem||'Write a problem or add a screenshot below.')}</strong></div>
  ...
  <div class="board-toolbar"><button id="pen" class="${tool==='pen'?'selected':''}" aria-pressed="${tool==='pen'}">✎ Pen</button><button id="eraser" class="${tool==='eraser'?'selected':''}" aria-pressed="${tool==='eraser'}">Eraser</button><button id="undo">Undo</button><button id="clear">Clear board</button><span>Your thinking goes here</span></div><canvas id="board" width="1200" height="775" aria-label="Math whiteboard. Draw with a mouse or pen, or use the ideas text box below."></canvas>
  ...
  $('#pen').onclick=()=>{tool='pen';$('#pen').classList.add('selected');$('#eraser').classList.remove('selected');$('#pen').setAttribute('aria-pressed','true');$('#eraser').setAttribute('aria-pressed','false');};
  $('#eraser').onclick=()=>{tool='eraser';$('#eraser').classList.add('selected');$('#pen').classList.remove('selected');$('#eraser').setAttribute('aria-pressed','true');$('#pen').setAttribute('aria-pressed','false');};
...
function drawBoard(){const c=$('#board');if(!c||!lesson)return;const ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);for(const s of lesson.strokes){ctx.globalCompositeOperation=s.tool==='eraser'?'destination-out':'source-over';ctx.strokeStyle='#1F3A78';ctx.lineWidth=s.tool==='eraser'?28:3.5;ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();s.points.forEach((p,i)=>{if(!i)ctx.moveTo(p[0]*c.width,p[1]*c.height);else ctx.lineTo(p[0]*c.width,p[1]*c.height);});if(s.points.length===1){const p=s.points[0];ctx.lineTo(p[0]*c.width+.1,p[1]*c.height+.1);}ctx.stroke();}ctx.globalCompositeOperation='source-over';}

...
async function dashboard() {
  ...
  main.innerHTML=`<section class="hero"><div><span class="eyebrow">A LITTLE CURIOSITY GOES A LONG WAY</span><h1>Big ideas start<br>with little steps.</h1><p>A space to think out loud, make mistakes, and figure things out together. Your work is right here.</p><button class="primary" id="new-main">＋ Start a math lesson</button></div><div class="hero-robot"><span class="spark">✦</span><span class="robot"><i></i><i></i></span></div></section>
  ${config.demo ? '<div class="demo-banner"><div><strong>Try Buddy, one step at a time.</strong><p>Offline demo · built-in examples · no microphone, API calls, or handwriting recognition.</p></div><button id="walkthrough">▶ Play workflow demo</button></div>' : !config.ready ? `<div class="demo-banner">Live tutor needs ${escapeHTML(config.key_name)} in the bridge environment file. Restart the service after adding it. You can still save work.</div>` : ''}
...
function renderLesson() {
  ...
  main.innerHTML=`<section class="lesson-head"><div><span class="eyebrow">${s.mode==='learn'?'LEARN A TOPIC':'WORK THROUGH YOUR PROBLEM'}${s.demo?' · OFFLINE DEMO':''}</span><h1>${escapeHTML(s.topic)}</h1><p>${escapeHTML(s.level)} <span class="slash">/</span> ${readableStage(s.stage)}</p></div><div class="toolbar-actions"><button id="history">Saved work</button><button id="export">Export</button><button id="finish">${ended?'Resume lesson':'Save & end'}</button></div></section>
  <div class="workspace"><section class="board-panel"><div class="problem-strip"><span class="eyebrow">${input?'YOUR PROBLEM':'LET’S TRY THIS'}</span><strong>${escapeHTML(s.problem||'Write a problem or add a screenshot below.')}</strong></div>
  ...
  <div class="board-toolbar"><button id="pen" class="${tool==='pen'?'selected':''}">✎ Pen</button><button id="eraser" class="${tool==='eraser'?'selected':''}">Eraser</button><button id="undo">↶ Undo</button><button id="clear">Clear board</button><span>Your thinking goes here</span></div><canvas id="board" width="1200" height="775" aria-label="Math whiteboard. Draw with a mouse or pen, or use the ideas text box below."></canvas>
  ...
  <aside class="tutor-panel"><div class="tutor-head"><span class="robot"><i></i><i></i></span><div><h3>Buddy is here.</h3><p>Let's take it one step at a time.</p></div></div><div class="tutor-feed" id="feed"></div><div class="tutor-controls"><button id="hint">✦ Give me a hint</button><button id="check">✓ Check my work</button><button id="step" class="primary">→ Show one step</button><button id="stuck">I don't know how to start</button></div><label class="supervise"><input type="checkbox" id="supervise" ${supervise?'checked':''}>Check after a pause (5 seconds)</label><p class="small supervise">Your writing stays on the board. Buddy's steps appear here.</p></aside></div>`;
  ...
  $('#pen').onclick=()=>{tool='pen';$('#pen').classList.add('selected');$('#eraser').classList.remove('selected');};
  $('#eraser').onclick=()=>{tool='eraser';$('#eraser').classList.add('selected');$('#pen').classList.remove('selected');};
  $('#undo').onclick=()=>{lesson.strokes.pop();drawBoard();markDirty();};
  $('#clear').onclick=()=>{lesson.strokes=[];drawBoard();markDirty();};
...
function bindCanvas(){const c=$('#board');c.onpointerdown=e=>{if(busy||lesson.stage==='ended')return;e.preventDefault();c.setPointerCapture(e.pointerId);drawing=true;stroke={tool,points:[point(e)]};lesson.strokes.push(stroke);drawBoard();};c.onpointermove=e=>{if(!drawing)return;stroke.points.push(point(e));drawBoard();};const end=()=>{if(!drawing)return;drawing=false;markDirty();};c.onpointerup=end;c.onpointercancel=end;}
function drawBoard(){const c=$('#board');if(!c||!lesson)return;const ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);for(const s of lesson.strokes){ctx.globalCompositeOperation=s.tool==='eraser'?'destination-out':'source-over';ctx.strokeStyle='#344c69';ctx.lineWidth=s.tool==='eraser'?28:3.5;ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();s.points.forEach((p,i)=>{if(!i)ctx.moveTo(p[0]*c.width,p[1]*c.height);else ctx.lineTo(p[0]*c.width,p[1]*c.height);});if(s.points.length===1){const p=s.points[0];ctx.lineTo(p[0]*c.width+.1,p[1]*c.height+.1);}ctx.stroke();}ctx.globalCompositeOperation='source-over';}
...
function work(){return {ideas:lesson.ideas,problem:lesson.problem,stuck:lesson.stuck,strokes:lesson.strokes,source_image:lesson.source_image,board_image:boardImage()};}

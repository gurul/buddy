...
| Apps and Ask Claude (`CC_BUDDY_MINIAPP=1`) | "Make me a habit tracker" builds a small app with `claude-opus-5-5` that opens inside Telegram and saves its data on the Mac. Each build is used in a headless phone-sized browser and sent back to Claude to fix what broke (up to 2 rounds), then arrives as a picture with an Open button. Apps can be changed, undone (10 versions kept), renamed, deleted (to a trash folder) and restored from the chat or the Apps tab. Each app runs sandboxed with a key to its own data only. The same Mini App has a Claude chat, streamed in full. Open it from the pinned message or `/apps`; the menu button stays the `/` commands. Owner-only (signed `initData`), spend tracked per day with no cap unless `CC_BUDDY_MINIAPP_DAILY_USD` is set, through a Cloudflare quick tunnel. See [the Mini App](docs/stackchan/miniapp.md). |

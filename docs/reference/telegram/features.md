<!-- source: https://core.telegram.org/bots/features · fetched 2026-09-23 -->
...
For even more flexibility, Mini Apps support custom interfaces with JavaScript that launch directly inside Telegram.  [Bot interface features](https://core.telegram.org/file/464001858/11318/ahAJjwERIX8.164875/ce1372cbf73e3ea94e)
...
Commands should be **as specific as possible** – for example `/newlocation` or `/newrule` **is better** than a `/new` command that then requires an additional parameter from the user like "*location*“ or ”*rule*".  [Bot commands shown in a list](https://core.telegram.org/file/464001775/10227/HCr0XgSUHrg.119089/c17ff5d34fe528361e)
...
The **menu button** can alternatively be used to launch a Mini App.
...
- **Streaming replies** let bots show progress while generating a response.
- **Topics in private chats** help organize long-running conversations with a bot.
...
Remember that inline functionality has to be enabled via [@BotFather](https://t.me/botfather), or your bot will not receive inline [Updates](https://core.telegram.org/bots/api#update).  [Inline query results from a bot](https://core.telegram.org/file/464001466/10e4a/r4FKyQ7gw5g.134366/f2606a53d683374703)

>

Examples of inline bots include [@gif](https://gif.t.me), [@bing](https://bing.t.me) and [@wiki](https://wiki.t.me). Mini App bots can also be used inline – try typing [@durgerkingbot](https://durgerkingbot.t.me) in any chat.
...
Mini Apps also support deep linking, for more information check out our [dedicated guide](https://core.telegram.org/bots/webapps#adding-bots-to-the-attachment-menu).
...
Check out rich messages to learn how to combine ephemeral personalized views with complex [structured content](https://core.telegram.org/bots/api#inputrichmessage) and embedded [buttons](https://core.telegram.org/bots/api#richmessagebutton).
...
In addition to bots sending private messages, users can send **ephemeral commands** to bots. When a command is configured as ephemeral, the user's initial message remains completely invisible to other group members and other bots, allowing for discreet interactions in public spaces.  [Private ephemeral message shown to one user](https://core.telegram.org/file/400780400146/2/NCkBKEh9JJ0.148639.jpg/abe830e0723e38a4f9)
...
- Topics in Private Chats keep separate projects or support cases organized – improving on established web-based AI chat UIs.
- Streaming Replies show progress while the agent works and allow users to stop a draft at any point.
- Rich Messages turn the final result into a structured report, checklist, comparison, explanation or interactive flow.
- Guest Bots and Inline Mode make specialized agents available where they are needed.
- Bot-to-Bot Communication and Managed Bots enable delegation, coordination and complex multi-agent setups.
- Business Bots allow account automation with explicit rights, while Mini Apps provide a richer workspace for configuration, approvals and dashboards.
...
- Use Mini Apps to bring complete services into Telegram.
...
Bots can use the Stars they receive to [increase message limits](https://telegram.org/blog/dynamic-video-quality-and-more#increased-message-limits-for-bots), [send gifts](https://core.telegram.org/bots/api#sendgift) to users or [accept rewards](https://telegram.org/blog/monetization-for-channels) in Toncoin.  [Telegram Stars used for bot transactions](https://core.telegram.org/file/400780400296/3/gxdJYNSAgsg.335683/d6be0165da6f79d605)
...
[Mini Apps](webapps) allow developers to create flexible interfaces that can be launched right inside Telegram – integrating seamlessly with the app and bringing complete services into the chat.
...
When opened from a [direct link](webapps#direct-link-mini-apps) in a group, mini apps can also use the `chat_instance` parameter to track the current context, supporting **shared usage** by multiple chat members – to create live whiteboards, group orders, multiplayer games and much more.

#### Mini App Previews

Developers can **upload screenshots** and **video demos** of their mini app right from the bot's **profile page** – giving users an overview of the app's features and functionality. These media previews will be shown to any user who **views your app** – like in the Mini App Store or via Search.  [Mini App screenshots and video previews](https://core.telegram.org/file/400780400954/6/9rSb8AM77Ss.158225/1c635ef877be7a6747)
...
1. Open the bot's settings in BotFather’s [MiniApp](https://t.me/Botfather?startapp) and enable *'Bot Management Mode'*.
...
Telegram bots also support Inline Mode, which allows sending requests to bots right from the input field — from any chat on Telegram.
...
- [Guide for physical goods and services](https://core.telegram.org/bots/payments)
  [Bot payment interface](https://core.telegram.org/file/464001393/101fc/SB_bFCLR0tg.130549/7ecf91aaa44737e8cb)
...
1. Use the `/setdomain` command in [@BotFather](https://t.me/botfather) to pair the bot with your website domain.
1. Configure your widget using [our dedicated tool](https://core.telegram.org/widgets/login#widget-configuration) and embed it on your website.

#### Inline Login

When users open your website via an **inline button**, you can use the [login_url](https://core.telegram.org/bots/api#loginurl) parameter as an alternative to login widgets. This way, you'll be able to [seamlessly authorize](https://telegram.org/blog/privacy-discussions-web-bots#meet-seamless-web-bots) them on your website or app before the page even loads.  [Inline login interface](https://core.telegram.org/file/464001287/105ea/23bO4ntH4tc.43741/c9624e36c3462b6540)
...
- [HTML5 Games Bot API Docs](https://core.telegram.org/bots/api#games)
  [HTML5 game interface](https://core.telegram.org/file/464001558/11b92/y_Fu9QQWtj0.141959/dc9db48f66800bf271)
...
- **Create** a new sticker pack via [createNewStickerSet](api#createnewstickerset). Set `sticker_type` to *regular* to create a sticker pack or to *custom emoji* to create a pack of custom emoji. Attach the [files](api#file) you wish to include in the pack as an array of [InputSticker](https://core.telegram.org/bots/api#inputsticker)
...
Custom emoji additionally support **adaptive colors** – they will always match the current context (e.g., white on photos, accent color when used as status, etc.); to enable this feature, use the `needs_repainting` parameter in [createNewStickerSet](api#createnewstickerset).

Once you're done creating and sharing your artwork, don't forget to check out our [remaining sticker methods](api#stickers) to find out how to [edit](api#setstickersetthumbnail), [delete](api#deletestickerfromset) and even [reorder](api#setstickerpositioninset) your pack.
...
Rich and ephemeral messages can be combined to build **personalized** and **interactive** flows within a single group or channel message.
...
The **token** is a string, like `<bot-token>`, which is required to authorize the bot and send requests to the Bot API. Keep your token secure and store it safely, it can be used by anyone to control your bot.
